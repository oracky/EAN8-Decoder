...
#	Data: 13.04.2020		      	     
#	Obsługiwany format: BMP mono-chrome   
# ------------------------------------------- 
...
# kody Start/Stop i kod modułu rozdzielającego	
start_sign:	.word 5
...
prepare_for_start:
	la $t9, code		# zapisanie adresu kodu wyjściowego
...
success_exit:
	...
	li $v0, 4
	syscall
	la $a0, code		# załadowanie odczytanego kodu
	li $v0, 4
	syscall
	j main_exit
error_exit:	
...
read_bmp:
	...
	sw $t0, filesize($a0)
	sw $a1, headeraddr($a0)
	lhu $t0, 10($a1) 	# przesunięcie obrazu wzg poczatku pliku
...
get_bit:
	# W $a0 adres bajtu, w którym znajduje się bit
	# W $a1 numer bitu, który ma odczytać
	# W $v0 funkcja zwraca wartość bitu
	li $a3, 1
	li $s3, 7
